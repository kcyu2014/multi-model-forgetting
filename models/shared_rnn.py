...
def dag_to_keys(dag, args):
    ...
    while True:
        ...
        for next_node in nodes:
            ...
            if next_id == args.num_blocks:
                leaf_node_ids.append(node_id)
                assert len(nodes) == 1, ('parent of leaf node should have '
                                         'only one child')
                continue
            keys.append(node_to_key((node_id, next_id, 'h')))
            keys.append(node_to_key((node_id, next_id, 'c')))
            q.append(next_id)
    return keys


def dag_to_genotype(dag,args):
    # keys = []
    ids = args.num_blocks * [-1,]
    funcs = args.num_blocks * [None,]
    ...
    while True:
        ...
        for next_node in nodes:
            ...
            if next_id == args.num_blocks:
                ...
                continue
            ids[next_id] = node_id
            funcs[next_id] = next_node.name
            q.append(next_id)

    return [(f, i + 1) for i,f in zip(ids, funcs)], [i + 1 for i in sorted(leaf_node_ids)]
...
class RNN(models.shared_base.SharedModel):
    ...
    def cell(self, x, h_prev, dag):
        ...
        while True:
            ...
            node_id = q.popleft()
            nodes = dag[node_id] # Dag is quite different from CNN dag
    ...
    def update_fisher(self, dag, epoch=0):
        """ a single dag
        return the count of entire fisher module to be updated.
        """
        return super(RNN, self).update_fisher(dag_to_keys(dag), epoch=epoch)

    #### New ones
    def to_device(self, device, dag):
        """Moves the parameters on the specified path to the device"""
        keys = dag_to_keys(dag, self.args)
        for k in keys:
            self.connections[k].to(device)
    ...
    def compute_weight_plastic_loss_with_update_fisher(self, dag):
        return super(RNN, self).compute_weight_plastic_loss_with_update_fisher(dag_to_keys(dag))
